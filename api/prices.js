export default async function handler(req, res) {
  ...
  const endpoints = ['https://api1.binance.com','https://api2.binance.com','https://api3.binance.com','https://api.binance.com'];
  ...
  // All Binance endpoints failed — try CoinGecko as fallback
  try {
    const ids = 'bitcoin,ethereum,solana,binancecoin,cardano,dogecoin,ripple,polkadot';
    const r = await fetch(`https://api.coingecko.com/api/v3/simple/price?ids=${ids}&vs_currencies=usd&include_24hr_change=true&include_24hr_vol=true`, { signal: AbortSignal.timeout(8000) });
    const data = await r.json();
    const map = { bitcoin:'BTCUSDT', ethereum:'ETHUSDT', solana:'SOLUSDT', binancecoin:'BNBUSDT', cardano:'ADAUSDT', dogecoin:'DOGEUSDT', ripple:'XRPUSDT', polkadot:'DOTUSDT' };
    const markets = Object.entries(data).map(([id, v]) => ({
      symbol: map[id],
      price: v.usd,
