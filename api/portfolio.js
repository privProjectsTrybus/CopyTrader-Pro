...
async function getBinanceAccount(apiKey, secret) {
  ...
  // Try every known Binance endpoint including non-US ones
  const endpoints = [
  ...
  for (const base of endpoints) {
    try {
      const r = await fetch(`${base}/api/v3/account?${queryString}&signature=${sig}`, {
        headers: { 'X-MBX-APIKEY': apiKey, 'Content-Type': 'application/json' },
        signal: AbortSignal.timeout(8000),
      });
      const text = await r.text();
      // Skip if HTML (geo-block) or 4xx geo errors
      if (text.startsWith('<') || text.startsWith('{    error')) continue;
      const data = JSON.parse(text);
      if (data.code === -2015 || data.code === -2014) throw new Error('Invalid API key — check Binance key/secret are correct');
  ...
  throw new Error('Geo-blocked on all Binance endpoints. Binance restricts access from Vercel/AWS servers. Solution: use a Binance API proxy or move backend to a non-AWS host.');
}
...
async function getBybitAccount(apiKey, secret) {
  ...
  // Bybit alternative endpoints that may not be on CloudFront
  const bybitEndpoints = [
    'https://api.bybit.com',
    'https://api.bytick.com',   // Bybit alternative domain
  ];

  for (const base of bybitEndpoints) {
    for (const accountType of ['UNIFIED', 'CONTRACT', 'SPOT']) {
      try {
        ...
        const text = await r.text();
        if (text.startsWith('<') || text.includes('CloudFront') || text.includes('blocked')) continue;
        ...
        if (data.retCode === 0) {
          ...
          if (coins.length > 0) return { coins, accountType, base };
          continue;
        }
        if ([10005, 10003, 10001, 33004].includes(data.retCode)) continue;
        throw new Error(`Bybit error ${data.retCode}: ${data.retMsg}`);
      } catch (e) {
        if (e.message?.includes('Bybit error')) throw e;
        continue;
      }
    }
  }
  throw new Error('Bybit geo-blocked via CloudFront from Vercel/AWS. Both Binance and Bybit block AWS server IPs. See README for proxy setup.');
}
